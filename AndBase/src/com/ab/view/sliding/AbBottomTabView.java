...
public class AbBottomTabView extends LinearLayout {
	...
	public static enum Mode{
		TABTOP, TABBOTTOM, TABONLY
	}
	
	private Mode mode = Mode.TABBOTTOM;
    ...
    public AbBottomTabView(Context context, AttributeSet attrs){
    	this(context, attrs, Mode.TABBOTTOM);
    }
	 ...
	 */
	public AbBottomTabView(Context context, AttributeSet attrs, Mode mode) {
		super(context, attrs);
...
//		addView(mTabLayout, new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
	...
	public void setMode(Mode mode){
		this.mode = mode;
		removeAllViews();
		if(mode == Mode.TABBOTTOM){
			addView(mViewPager,new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,0,1));
			addView(mTabLayout, new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
		}
		else if(mode == Mode.TABTOP){
			addView(mTabLayout, new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
			addView(mViewPager,new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,0,1));
		}
		else if(mode == Mode.TABONLY){
			addView(mTabLayout, new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
	...
	public void addItemViews(List<String> tabTexts,List<Fragment> fragments){
		...
		if(fragments!=null && fragments.size() != 0)pagerItemList.addAll(fragments);
	...
	public void addItemView(String tabText,Fragment fragment){
		...
		if(fragment!=null)pagerItemList.add(fragment);
		mFragmentPagerAdapter.notifyDataSetChanged();
		notifyTabDataSetChanged();
	}
